...
boutonPdf.addEventListener("click", () => {
  ...
  Date.parse(dateRupture.value) - Date.parse(dateEmbauche.value) > 0
    ? exportAnciennetePdf()
    : console.log("Pas d'ancienneté"); //Trigger pour exporter l'ancienneté
  affichageSalaireReference && affichageSalaireReference.innerText != ""
    ? exportSalairePdf()
    : console.log("Pas de salaire de reference"); //Trigger pour exporter le salaire de référence
  ...
  function exportDelaisPdf() {
    ...
    selector.value === ""
      ? (textToSet = "pas de selection de délais de procédure")
      : (textToSet = selector.options[selector.selectedIndex].text);
    doc.text(textToSet, 10, (espace += 10));

    document.getElementById("startDate").value === ""
      ? doc.text("Date de départ non saisie", 10, (espace += 20))
      : doc.text(
          document.querySelector('[for="startDate"]').innerHTML +
            new Date(document.getElementById("startDate").value).toLocaleDateString("fr"),
          10,
          (espace += 20),
        );

    calendrier === null ? doc.text("", 10, (espace += 20)) : doc.text(calendrier.innerText, 10, (espace += 20));
    textAdded = true;
  }
  ...
  function exportAnciennetePdf() {
    ...
    doc.text(
      "Période d'emploi du " +
        new Date(document.getElementById("dateEmbauche").value).toLocaleDateString("fr") +
        " au " +
        new Date(document.getElementById("dateRupture").value).toLocaleDateString("fr"),
      10,
      (espace += 10),
    );
